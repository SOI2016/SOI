...
int SOI_Create(const char *shm_name, int Payload_Size, int flag)
{
	...
	if(flag != SOI_ATTACH)
	{
		...
		if (ftruncate(shm_fd, SOI_SHMSIZE) == -1)
		{
			printf("shm_open error\n");
			exit(1);
		}
		...
		Shm_Ptr.shm_mem = mmap(0, SOI_SHMSIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
	}
	else
	{
		Shm_Ptr.isAttach = SOI_ATTACH;
		/* Create the shared memory object */ 
	...
	cirque_ds = Shm_Ptr.shm_mem + (Payload_Size * ((SOI_SHMSIZE/Payload_Size) - 1));
	Cir_Que_Ptr = (CQUEUE *)cirque_ds;
	Shm_Ptr.curr_seg = Shm_Ptr.shm_mem;

	if(Shm_Ptr.isAttach != SOI_ATTACH)
	{
		init_cir_Buf(Cir_Que_Ptr, shm_fd);
...
int SOI_Send(int shm_id, void *send_buf)
{
	...
	else
	{
		...
		else
		{
			...
			memcpy (Shm_Ptr.curr_seg, send_buf, Cir_Que_Ptr->payload_size);
			Shm_Ptr.curr_seg += Cir_Que_Ptr->payload_size;
...
int SOI_Recv(int shm_id, void *recv_buf, int msg_flg)
{
	...
	else
	{
		...
		else
		{
			...
			{
				memcpy (recv_buf,(Shm_Ptr.shm_mem + (Cir_Que_Ptr->payload_size * Cir_Que_Ptr->rear)), Cir_Que_Ptr->payload_size);
				Cir_Que_Ptr->front	= 0; 
				Cir_Que_Ptr->rear	= -1;
				Cir_Que_Ptr->count	= 0;
				Shm_Ptr.curr_seg	= Shm_Ptr.shm_mem;
			...
			{
				memcpy (recv_buf,(Shm_Ptr.shm_mem + (Cir_Que_Ptr->payload_size * Cir_Que_Ptr->front)), Cir_Que_Ptr->payload_size);
...
int SOI_Destroy(int shm_id, const char *shm_name)
{
	...
	else
	{
		...
		rc = munmap(Shm_Ptr.shm_mem, SOI_SHMSIZE);
		if (rc == -1) 
